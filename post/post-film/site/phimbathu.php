<?php
	...
	function parseInfo($url, $encode = true) {
		...
		$e->episode = Array();
		$link_btn = constant('domain').$page->find('div.poster', 0)->find('ul.buttons', 0)->find('li', 0)->find('a', 0)->getAttribute('href');
		$content = file_get_contents($link_btn);
		$frl = str_get_html($content);
		if (strpos($content, '<div class="list-server" id="list-server">')) {
			$epsID = (object) Array();
			$epsID->name = 'HD';
			$epsID->list = Array();
			$tepSD = (object) Array();
