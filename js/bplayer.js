...
function BPlayer(element, options) {
    ...
    this.setUpVideo = function() {
        var self = this;        
        if(playAds == 0) {
            var firstSource = self.isMobile == 0 ? [{file: 'http://bilutv.com/uploads/1.mp4',type:'mp4',label: '360p',default: true}] : self.sources;
			...
			//self.checkClientIp();
            if(reloadTimes <= 7) {
				var tmpAdvertising = cloneObject(self.advertising);
                delete tmpAdvertising["schedule"]["adbreak1"];
                self.playerInstance.setup({
            ...
            if(reloadTimes <= 5) {
                ...
                self.setUpVideoEvent();
            } else if(reloadTimes <= 7 && self.options['linkBackup'] != "") {
                $.getJSON(self.options['linkBackup'], function( data ) {
    ...
    this.setUpVideoEvent = function() {
        ...
        self.playerInstance.on('ready', function () {
			...
			if(reloadTimes > 5) {
				var clientip = getCookie("c_info");
				if(clientip == 4) {
					createCookie("c_info", 6, 10800);
				} else {
					createCookie("c_info", 4, 10800);
				}
			}
            ...
            if($("#btn_autonext").hasClass('active') || true) {
                self.playNextLink();
            }
