...
function BPlayer(element) {
	...
	this.getOriginalSources = function(sources) {
		if(typeof(sources) === 'object') {
			for (var index in sources) {
				if(typeof sources[index]["links"] != 'undefined' && sources[index]["links"].length > 0) {
					for (var itemIndex in sources[index]["links"]) {
						if(typeof sources[index]["links"][itemIndex]['file'] != 'undefined') {
	...
	this.getPlayerLink = function() {
		...
		var score = {'dr': 0,'sb': 3, 'cs': 2,'st': 5, 'op': 7,'sm': 8,'gd': 9,'yt': 10, 'gs': 11,'gp': 12,'ga': 12};
		...
		if(this.sourceLinksBk[0]) {
			...
			if(this.sourceLinks[2]) this.sourcesBk4 = this.sourceLinks[2]["links"];
		} else {
			...
			if(this.sourceLinks[3]) this.sourcesBk4 = this.sourceLinks[3]["links"];
		}
	};
    ...
    this.setUpVideo = function() {
        ...
        if(self.playAds < self.maxAds) {
			...
			if(self.reloadTimes <= 6) {
				if(self.reloadTimes >= 2 && self.sourcesBk2) self.sources = self.sourcesBk2;
				if(self.reloadTimes >= 4 && self.sourcesBk3) self.sources = self.sourcesBk3;
				if(self.reloadTimes >= 6 && self.sourcesBk4) self.sources = self.sourcesBk4;
				...
				} else if(self.sources[0].type == "links") {
					$.getJSON(self.sources[0].file, function(data) {
    ...
    this.setUpVideoEvent = function() {
        self.playerInstance.on('ready', function () {
            ...
            self.playerInstance.setVolume(80);
    ...
    this.playNextLink = function () {
		...
		console.log("xxx");
        if ($elm.length) {
            var next_link = $elm.closest("li").next().find("a").attr('href');
