<?php
	...
	$file = '';
	if(isset($_POST['sources']) && isset($_POST['modelId'])){
		if(!empty($_POST['sources']) && !empty($_POST['modelId'])){
			$file = GibberishAES::dec($_POST['sources'],constant("key").$_POST['modelId']);
		}
	}
	...
	// 		$Result[$i]['file'] = GibberishAES::dec($StreamUrl[$i], constant("key").$EpisodeID);
	// 		var_dump(constant("key").$EpisodeID);die;
	// 		$Result[$i]['label'] = $Resolution[$i]."p";
